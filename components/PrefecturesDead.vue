<template>
  <div>
    ...
    <input id="graphChoice4" type="radio" name="selectGraph2" value="2" />
  ...
  created() {
    this.$store.state.prefecturesData.forEach((data) => {
      if (data.pref_code === this.selected_pref_code) {
        ...
        this.chartdata.datasets[0].data.push(data.total_dead)
      }
    })
  },
  methods: {
    chengeGraph(selectedPrefCode) {
      ...
      this.$store.state.prefecturesData.forEach((data) => {
        if (data.pref_code === Number(selectedPrefCode)) {
          ...
          newChartDataDatasets.push(data.total_dead)
