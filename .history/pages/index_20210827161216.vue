...
export default{
    async fetch({ store }) {
      ...
      .then(res => {
        ...
        const payload = {
          totalDeath: necessaryData[6].ne,
          comparison_yesterday: (res.data.slice(-1).ndeaths) - (res.data.slice(-2).ndeaths),
          comparison_oneWeek: (res.data.slice(-1).ndeaths) - (res.data.slice(-7).ndeaths)
        }
        store.dispatch("fetchTotalDeath",necessaryData)
      })
  },
  // computed:{
  //   data(){
