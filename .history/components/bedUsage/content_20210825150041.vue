<template>
  <div>
    ...
    <div class="flex items-center justify-center">
      ...
      >
        <div v-for="(data, index) in dataArry" :key="data.id">
          <div
            class="relative py-2 px-2 rounded-md w-36"
            :class="
    ...
    <p class="text-white m-5 pt-11 text-xs text-center">療養状況等及び入院患者受入病床数等に関する調査についてのJSONデータ情報提供:新型コロナウイルス対策ダッシュボード</p>
    <p class="whitespace-no-wrap text-white m-5 pb-20 text-xs text-center md:text-sm">療養状況等及び入院患者受入病床数等に関する調査についてのJSONデータ情報提供:新型コロナウイルス対策ダッシュボード</p>
  </div>
</template>
<script>
